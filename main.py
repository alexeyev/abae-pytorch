...
def main(cfg):
    ...
    model = ABAE(
        ...
        init_aspects_matrix=get_centroids(
            w2v_model, aspects_count=cfg.model.aspects_number
        ),
    )
    ...
    for t in range(cfg.model.epochs):
        ...
        data_iterator = read_data_tensors(
            os.path.join(hydra.utils.get_original_cwd(), cfg.data.path),
            os.path.join(hydra.utils.get_original_cwd(), cfg.embeddings.path),
            batch_size=cfg.model.batch_size,
            maxlen=cfg.model.max_len,
        )
        ...
        for item_number, (x, texts) in enumerate(data_iterator):
            ...
            negative_samples = torch.stack(
                tuple(
                    [
                        x[torch.randperm(x.shape[0])[: cfg.model.negative_samples]]
            ...
            if item_number % cfg.model.log_progress_steps == 0:
                logger.info(
                    "%d batches, and LR: %.5f"
                    % (item_number, optimizer.param_groups[0]["lr"])
                )
                ...
                try:
                    torch.save(model, "abae_%.2f_%06d.bin" % (loss.item(), item_number))
                except Exception:
