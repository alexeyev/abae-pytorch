...
class ABAE(torch.nn.Module):
    ...
    def forward(self, text_embeddings, negative_samples_texts):
        ...
        recovered_emb = torch.matmul(
            self.aspects_embeddings, aspects_importances.unsqueeze(2)
        ).squeeze()
        ...
        max_margin = torch.max(
            reconstruction_triplet_loss, torch.zeros_like(reconstruction_triplet_loss)
        ).unsqueeze(dim=-1)
    ...
    def _reconstruction_loss(text_emb, recovered_emb, averaged_negative_emb):
        ...
        positive_dot_products = torch.matmul(
            text_emb.unsqueeze(1), recovered_emb.unsqueeze(2)
        ).squeeze()
        negative_dot_products = torch.matmul(
            averaged_negative_emb, recovered_emb.unsqueeze(2)
        ).squeeze()
        reconstruction_triplet_loss = torch.sum(
            1 - positive_dot_products.unsqueeze(1) + negative_dot_products, dim=1
        )
